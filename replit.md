...
✓ **Warranty Management (January 2026)**: Users can add warranty information to receipts from the receipt detail page. The system automatically calculates warranty expiry dates from the receipt date plus user-specified duration in months. Features include active/expiring/expired status indicators, warranty list page at `/warranties` showing all warranties organized by status, edit/delete functionality, and secure API endpoints with user ownership enforcement at both route and storage layers.
